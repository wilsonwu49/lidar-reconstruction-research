%% extract_fft_results.m
%% Extracts FFT Lookup results from benchmark_results.mat into a separate file
clear; clc;
...
for di = 1:numel(allResults)
    fprintf('  %d. %s  methods: %s\n', di, allResults(di).tag, ...
        strjoin(allResults(di).methods, ', '));
end
...
for di = 1:numel(allResults)
    ...
    %% count how many frames actually have data
    nValid = sum(isfinite(entry.rmse));
    fprintf('  Dataset %s: %d / %d frames have FFT data\n', ...
        D.tag, nValid, numel(D.frameNums));
